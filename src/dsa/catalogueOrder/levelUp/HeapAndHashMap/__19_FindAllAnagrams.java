...
public class __19_FindAllAnagrams {
    ...
    public List<Integer> findAnagrams(String str, String pattern) {
        ...
        for (int i = 0; i <= str.length() - windowSize; i++) {
            while (j - i + 1 <= windowSize) {
                ...
                if (j - i + 1 == windowSize) {
                    if (sMap.equals(pMap)) { // we could write our own hashMapCompare function but equals in hashmap is overridden to provide comparision
                        result.add(i);
                    }
